...
- **Manual scroll + click-to-seek** — The grid is freely scrollable. Tapping/clicking any bar seeks playback to the start of that bar, including across section boundaries (the correct loop audio is loaded automatically). During playback, manual scrolling temporarily pauses auto-scroll (re-engages after 5s or on next bar click). Works while playing or paused.
